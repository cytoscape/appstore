     ...
     ================================================================
        */
        ...
        function get_app_status(fullname, callback) {
                ...
                {'type': 'GET',
                 'success': function(result) { if (result.includes('Installed')) {callback(true);} else {callback(false);} },
                 'error': function() { callback(false); }},
                        callback);
        }
        ...
        function get_app_updates(fullname, callback) {
                ...
                {'type': 'GET',
                 'success': function(result) { if (result.includes(fullname)) {callback(true);} else {callback(false);} },
                 'error': function() { callback(false); }},
                        callback);
        }
                                ...
                                get_app_status(app_fullname, function(app_status) {
                                        if (!app_status) {
                                                ...
                                                get_app_updates(app_fullname, function(app_updates){
                                                        if(!app_updates){set_install_btn_to_installed();
                                                         } else {
